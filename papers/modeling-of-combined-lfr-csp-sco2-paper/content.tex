...
The values for the three non-charging cycle models with a focus on electrical generation, 2-cycle configuration composed of C-LFR-ON and C-CSP-ON, C-1HTR1T-ON, and C-2HTR3T-ON, are displayed in Table \ref{tab-noncharg}. \mw{this sentence is hard to parse. What is meant by ``focus on electrical generation?''}
...
\bl{need to comment on how this interacts with the choice of cycle configuration. If the LFR temperature is 400C and cold TES is 390C then CSP inlet must be 380C and this requires 2HTR3T or separate cycles. If 1HTR1T is used, then 410C+ is needed which implies an efficiency penalty}
