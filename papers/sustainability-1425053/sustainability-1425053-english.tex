...
\Author{\hl{Brian White} %Please carefully check the accuracy of names and affiliations. Changes will not be possible after proofreading.
 $^{1}$, Michael Wagner~$^{1}$\orcidB{}, Ty Neises~$^{3}$, Cory Stansbury~$^{4}$ and Ben Lindley~$^{2,}$*\orcidA{}}
...
\address{%
$^{1}$ \quad Department of Mechanical Engineering, University of Wisconsin---Madison, 1415 University Drive, \mbox{Madison, WI 53706, USA;} bwhite24@wisc.edu (B.W.); mjwagner2@wisc.edu (M.W.)\\
...
A CSP has an array of mirrors concentrating solar rays towards a %\replaced[comment={Furthering explanation of the CSP [3,1]}]
...
\end{equation}
where $C_{avg}$ is the average capacitance rate across the hot or cold stream and $c_{in}$ and $c_{out}$ is the specific heat at the inlet and outlet respectively. %\replaced[comment={Improving counter-flow hx explanation [1,1]}]
