...
for file in fnames2:
    ...
    if file not in ['synData.csv','synDataWind.csv','dataSet.csv', '102574_35.93_-115.26_2004.csv', 'romMeta.csv','solar_data_without_clearsky.csv']:
        ...
        if file[:31] == 'solar_data_without_clearsky1.csv':
            ...
            #specify headers of interest
            headers3 = ["Time", "DNI","DHI", "Temp", "Precip", "RH", "Wind"]
            original_data = pd.read_csv(filename_orig ,names=headers3)
        elif file[:11] == 'synDataWind':
            headers = ["Time","Wind"]
...
plt.ylabel('Standard Deviation / m$^-1')
ax = plt.gca()
ax.axes.xaxis.set_ticklabels(['Real Data', 'Synthetic Data'])
ax.annotate(
    'Original Data',
