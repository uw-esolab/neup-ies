...
# plt.plot(x,np.cos(np.radians(sz.values)))
# plt.show()

dhienv = np.zeros(len(cskyn.values))

for i in range(len(cskyn.values) -1):
    dhienv[i] = 400 + 140*np.cos(2*np.pi*((x[i]-(max(x)/2)+25000)/max(x)))
...
for i in range(len(RH)):
    ...
    #create a second list for the corrected data to be added to
S3 = np.zeros(len(cskyh.values))

# Convert back to DNI. 
for i in range(len(cskyh.values) -1):
    # Basic transform
    dd = float(GHI[i]) - float(DNI[i])*np.cos(np.radians(sz.values[i])) + cskyh.values[i]
    # Constrain values to be physical
    dd = max(0,dd)
    if cskyh.values[i] == 0:
        dd = 0
...
oDHI = np.array(oDHI[1:])
oDHI = oDHI.astype(float)
...
plt.plot(x,S3, color='r')
plt.plot(ox,oDHI, color='b')
plt.plot(x,cskyh.values)
plt.xlim(200000, 220000)
plt.title("DHI over course of two weeks for Las Vegas")
plt.xlabel("Time / minutes")
plt.ylabel("DHI")
plt.legend(["Synthetic","Original","Clearsky"])
plt.show()

#plt.plot(ox,solar_data)
plt.plot(x,S3, color='r')
plt.plot(ox,oDHI, color='b')
plt.plot(x,dhienv, color='g')
#plt.xlim(100000, 120000)
plt.title("DHI over course of year for Las Vegas")
plt.xlabel("Time / minutes")
plt.ylabel("DHI")
plt.legend(["Synthetic","Original"])
plt.show()
...
plt.hist(oDHI, bins=20,facecolor = 'none', edgecolor='blue', label = 'Original data', zorder = 101)
plt.hist(S3, bins=20,facecolor = 'none', edgecolor='red', label = 'Synthetic data', zorder = 101)
plt.legend()
plt.title("Distribution for GHI")
plt.show()