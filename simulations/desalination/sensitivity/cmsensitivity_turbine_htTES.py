...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  4 13:22:21 2023

@author: elizabethkeith
"""

"This model generates data for different high-temperature TES prices ranging from 25% base case to 200% base cases + different turbine sizes ranging from base case to 200% base case"
...
W_dot_nom                                         = 494  
turbine_sensitivity                               = [W_dot_nom, 1.25*W_dot_nom, 1.50*W_dot_nom, 1.75*W_dot_nom, 2.0*W_dot_nom]
desal_price_per_kg                                = 0.0021
cost_molten_salt_TES_kWh                          = 29.8  
high_temp_TES_sensitivity                         = [0.25*29.8, 0.5*29.8, 0.75*29.8, 1*29.8, 1.25*29.8, 1.50*29.8, 1.75*29.8, 2*29.8]



for j in high_temp_TES_sensitivity:
    
    counter_turbine                                   = 1

    for i in turbine_sensitivity:
        ...
        model.P_dist                                  = pyomo.Param(initialize=desal_price_per_kg)                                                           #[$/kg]           Price at which distillate is sold at time t
        ...
        model.C_cs                                    = pyomo.Param(initialize=j)                                    #[$/kWh]          Cost associated with cycle thermal energy storage
        ...
        def objective(model):
            ...
            return sum([model.P_elec[t]*model.w_dot[t]*model.Delta_t + model.P_dist*model.v_dot[t]*3600*model.Delta_t for t in model.T]) \
        ...
        title_str = 'turbine+htTES'
        title_str = title_str + str(counter_turbine) + '_' + str(counter_htTES) + '.csv'
