...
DeltaH_dtes               = 421.3 #kJ/kg  (h(5bar,140C) - h(5bar,40C))
...
cost_desal_TES_kWh        = 10.6  # [$/KWh-th], scaled from German TES
...
desal_price_per_kg        = desal_price_acre_foot/cubic_meter_per_acre_foot*specific_volume_H2O #[$/kg]
...
temperature_schedule      = pd.read_csv("./simulations/desalination/Diablo_Canyon_Temps_2021.csv", usecols=["Temperature"]) #temperatures in Diablo Canyon for TMY
...
model.K_chpt         = pyomo.Param(initialize=0.7*1.1*W_dot_gen/m_dot_csteam)    #[MW/(kg/s)]  Conversion constant for HPT steam mass flow to power in the cycle
model.K_clpt         = pyomo.Param(initialize=0.3*1.1*W_dot_gen/m_dot_esteam)   #[MW/(kg/s)]  Conversion constant for LPT extraction steam mass flow to lost power in the cycle
...
model.C_cs           = pyomo.Param(initialize=cost_molten_salt_TES_kg) #[$/kg] Cost associated with cycle thermal energy storage
model.C_ds           = pyomo.Param(initialize=cost_desal_TES_kg) #[$/kg] Cost associated with desal thermal energy storage
...
def objective(model):
    ...
    return sum([model.P_elec[t]*model.w_dot[t]*model.Delta_t + model.P_dist[t]*model.v_dot[t]*3600*model.Delta_t for t in model.T]) \
         ...
         - (model.C_cs*model.M_cm_max) - (model.C_ds*model.M_dm_max)
...
# print(model.display())
print(results)
outlabs = ['time', 'P_elec','m_ch', 'm_dh', 'w_dot', 'v_dot', 'm_dot_cs','m_dot_hpt','m_dot_e','m_dot_ds', 'M_cm_max', 'M_dm_max']
