...
Q_dot_LFR                         = 950                                                                               #[MW]             Heat input from LFR
W_dot_gen                         = 494                                                                               #[MW]             Nominal power output
eta_cycle_ref                     = 0.52                                                                              #[-]              Nominal efficiency
cp_salt                           = 1.5                                                                               #[kJ/kg-K]        Heat capacity of molten nitrate salt
DeltaT_ctes                       = 565-330                                                                           #[K]              Temperature difference between cold and hot high-temperature storage
DeltaH_csteam                     = 2126                                                                              #[kJ/kg]          Enthalpy difference across salt-to-steam heat exchanger (h(150bar,555C) - h(150bar,300C))
DeltaH_dsteam                     = 2494                                                                              #[kJ/kg]          Enthalpy (h(4.8bar,x=.98) - h(4.5bar,50C))  # steam extracted from turbine
...
m_dot_salt                        = Q_dot_LFR*1e3 / (cp_salt * DeltaT_ctes)                                            #[kg/s]          Nominal mass flow of salt 
m_dot_csteam                      = W_dot_gen/eta_cycle_ref*1e6 / (DeltaH_csteam*1e3)                                  #[kg/s]          Nominal mass flow to HPT     

f_lpt                             = 0.53                                                                               #[-]             Nominal fraction of steam flow that makes it to LPT stage 3

m_dot_esteam                      = m_dot_csteam*f_lpt                                                                 #[kg/s]          Nominal extraction flow rate
m_dot_dtes                        = m_dot_esteam*DeltaH_dsteam/DeltaH_dtes                                             #[kg/s]          Nominal flow rate of water into the desal storage tank


distillate_price_schedule         = [desal_price_per_kg for i in range(N)]
...
model.C_c_ramp                     = pyomo.Param(initialize=43.75)                                                     #[$/Delta MW]     Cost associated with change in electric power production
...
model.W_dot_max                    = pyomo.Param(initialize=W_dot_gen)                                                 #[MW]             Maximum power ouput
model.W_dot_min                    = pyomo.Param(initialize=W_dot_gen*0.25)                                            #[MW]             Minimum power output
model.V_dot_min                    = pyomo.Param(initialize=m_dot_dtes*model.K_d()*0)                                  #[kg/s]           Minimum desalination output
...
#       Variables
#  ==================================================
model.m_ch                         = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)                                  #[kg]             Inventory in cycle-side hot storage at time t
model.m_dot_cs                     = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)                                  #[kg/s]           Mass flow drawn from cycle-side hot storage at time t
model.m_dot_hpt                    = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)                                  #[kg/s]           Mass flow produced in the cycle at the high pressure turbine inlet at time t
model.m_dot_e                      = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)                                  #[kg/s]           Mass flow produced extracted and sent to the desal heat exchanger at time t
model.m_dh                         = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)                                  #[kg]             Inventory in the desal-side hot storage at time t
model.m_dot_ds                     = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)                                  #[kg/s]           Mass flow consumed by the desal system at time t
model.w_dot                        = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)                                  #[MW]             Electric power produced by the turbine system at time t
model.v_dot                        = pyomo.Var(model.T, domain=pyomo.NonNegativeReals)                                  #[kg/s]           Distillate produced by the desal system at time t
model.w_dot_Delta_up               = pyomo.Var(model.T-[1], domain=pyomo.NonNegativeReals)                              #[Delta MW]       Positive change in electric power produced at time t relative to the previous time step
model.w_dot_Delta_dn               = pyomo.Var(model.T-[1], domain=pyomo.NonNegativeReals)                              #[Delta MW]       Negative change in electric power produced at time t relative to the previous time step
...
df_out.to_csv('results2.csv', header=True)