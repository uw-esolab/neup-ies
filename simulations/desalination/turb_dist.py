"This model generates data for different low-temperature TES prices ranging from 10% base case to 200% base cases + different turbine sizes ranging from base case to 200% base case."
...
price_dist                                        = 0.0021 
sens_dist                                         = [0.1*price_dist, 0.2*price_dist, 0.3*price_dist, 0.4*price_dist, 0.5*price_dist, 0.6*price_dist, 0.7*price_dist, 0.8*price_dist, 0.9*price_dist, 1.0*price_dist, 1.1*price_dist, 1.2*price_dist, 1.3*price_dist, 1.4*price_dist, 1.5*price_dist, 1.6*price_dist, 1.7*price_dist, 1.8*price_dist, 1.9*price_dist, 2.0*price_dist]



for dist in sens_dist:
    ...
    for turb in sens_turb:
        ...
        # cost of low-temperature storage ($/kWh)
        cost_dtes                                     = 20   
        ...
        model.P_dist                                  = pyomo.Param(initialize=dist)                                                        #[$/kg]           Price at which distillate is sold at time t
        ...
        outlabs = ['t', 'P_dist', 'W_dot_gen',  'm_ch', 'm_dh', 'w_dot', 'v_dot', 'm_dot_cs','m_dot_hpt','m_dot_e','m_dot_ds', 'M_cm_max', 'M_dm_max', 'V_dot_max','C_desal']
        ...
        title_str = 'turb' + str(10*(9+counter_turb)) + '_dist' + str(10*counter_dist) + '.csv'
