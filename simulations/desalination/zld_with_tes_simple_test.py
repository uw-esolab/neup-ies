...
N            = 47                                         # units: --,      number of time steps (1 year)
...
Schedule_effic           = pd.read_csv('simple_sched_eff.csv', usecols=['Efficiency']).to_numpy().flatten()

Schedule_elec            = pd.read_csv('simple_test_prices.csv', usecols=['LMP']).to_numpy().flatten()
...
def safe_val(v, fmt="{:.2f}"):
    """Return value of Var/Param/Expression, or 'not solved' if undefined."""
    try:
        val = pyomo.value(v)       # works for Var, Param, Expression
...
def time_average(m, var, p):
    return sum(var[t,p] for t in m.T) / len(m.T)

def time_average_2D(m,var,q,p):
    return sum(var[t,q,p] for t in m.T) / len(m.T)

model.v_dot_in_avg   = pyomo.Expression(model.processes, rule=lambda m,p: time_average(m, m.v_dot_in,   p))
model.v_dot_conc_avg = pyomo.Expression(model.processes, rule=lambda m,p: time_average(m, m.v_dot_conc, p))
model.v_dot_dil_avg = pyomo.Expression(model.processes, rule=lambda m,p: time_average(m, m.v_dot_dil, p))
model.q_used_avg = pyomo.Expression(model.processes,rule=lambda m,p: time_average(m,m.q_used,p))


model.v_dot_link_avg = pyomo.Expression(model.links,rule=lambda m,q,p: time_average_2D(m,m.v_dot_link,q,p))
...
for p in model.processes:
    ...
    print(f"  v̇_in                = {safe_val(model.v_dot_in_avg[p])} m³/h")
    print(f"  v̇_conc              = {safe_val(model.v_dot_conc_avg[p])} m³/h")
    print(f"  v̇_dil               = {safe_val(model.v_dot_dil_avg[p])} m³/h")
    print(f"  Heat used           = {safe_val(model.q_used_avg[p])} kW-th")
...
for (q, p) in model.links:
    ...
    v = nval(model.v_dot_link_avg[q, p])
...
print("\n================== OBJECTIVE VALUE ==================")
try:
    print(f"  Profit = {pyomo.value(model.obj):,.2f} $")
except:
