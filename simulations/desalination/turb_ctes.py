...
"This model generates data for different high-temperature TES prices ranging from 10% base case to 200% base cases + different turbine sizes ranging from base case to 200% base case."
...
W_dot_nom                                         = 494  
sens_turb                                         = [W_dot_nom, 1.1*W_dot_nom, 1.2*W_dot_nom, 1.3*W_dot_nom, 1.4*W_dot_nom, 1.5*W_dot_nom, 1.6*W_dot_nom, 1.7*W_dot_nom, 1.8*W_dot_nom, 1.9*W_dot_nom, 2.0*W_dot_nom]
cost_ctes                                         = 29.8  
sens_ctes                                         = [0.1*cost_ctes, 0.2*cost_ctes, 0.3*cost_ctes, 0.4*cost_ctes, 0.5*cost_ctes, 0.6*cost_ctes, 0.7*cost_ctes, 0.8*cost_ctes, 0.9*cost_ctes, 1.0*cost_ctes, 1.1*cost_ctes, 1.2*cost_ctes, 1.3*cost_ctes, 1.4*cost_ctes, 1.5*cost_ctes, 1.6*cost_ctes, 1.7*cost_ctes, 1.8*cost_ctes, 1.9*cost_ctes, 2.0*cost_ctes]



for ctes in sens_ctes:
    
    counter_turb                                      = 1

    for turb in sens_turb:
        ...
        price_dist                                    = 0.0021                                                                            #[$/kg]            #price of desalinated water in 2021 in San Diego County from:(https://www.sdcwa.org/wp-content/uploads/2020/11/desal-carlsbad-fs.pdf)

        
        m_dot_salt                                    = Q_dot_LFR*1e3 /(cp_salt * DeltaT_ctes)                                           #[kg/s]
        m_dot_csteam                                  = W_dot_nom/eta_cycle_ref*1e6 /(DeltaH_csteam*1e3)                                 #[kg/s]
        ...
        model.C_c_ramp                                = pyomo.Param(initialize=43.75*W_dot_nom/m_dot_csteam)                                #[$/Delta kg/s]   Cost associated with change in electric power production, from Gabriel's paper
        ...
        model.K_chpt                                  = pyomo.Param(initialize=W_dot_nom/m_dot_csteam)                                      #[MW/(kg/s)]      Conversion constant for HPT steam mass flow to power in the cycle
        model.K_clpt                                  = pyomo.Param(initialize=W_dot_nom/m_dot_esteam)                                      #[MW/(kg/s)]      Conversion constant for LPT extraction steam mass flow to lost power in the cycle
        ...
        model.C_cs                                    = pyomo.Param(initialize=ctes)                                                        #[$/kWh]          Cost associated with cycle thermal energy storage
        model.C_ds                                    = pyomo.Param(initialize=cost_dtes)                                                   #[$/kWh]          Cost associated with desal thermal energy storage
        ...
        def constr_cmass(model, t):
            return model.w_dot[t]                    == ((model.eta[t]*W_dot_nom)/m_dot_csteam) * (model.alpha*model.m_dot_hpt[t] - model.beta*model.m_dot_e[t] - model.K_cint*m_dot_csteam)
        ...
        outlabs = ['t', 'C_ctes', 'W_dot_gen',  'm_ch', 'm_dh', 'w_dot', 'v_dot', 'm_dot_cs','m_dot_hpt','m_dot_e','m_dot_ds', 'M_cm_max', 'M_dm_max', 'V_dot_max','C_desal']
        ...
        title_str = 'turb' + str(10*(9+counter_turb)) + '_ctes' + str(10*counter_ctes) + '.csv'
