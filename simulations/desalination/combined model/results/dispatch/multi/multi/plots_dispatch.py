...
for j in ['desal', 'ltTES', 'htTES']:
    ...
    for i in range(1,6):
        ...
        for k in range(1,9):
            ...
            tit = 'turbine+' + j + str(i) + '_' + str(k) + '.csv'
            dat = pd.read_csv(tit)
...
for j in ['desal','ltTES', 'htTES']:
    ...
    for i in range(1,6):
        ...
        for k in range(1,9):
            ...
            lns1      = ax1.plot(dat.t, dat.v_dot, linewidth=0.1, color='red', label='Distillate')
            ax2       = ax1.twinx()
            lns2      = ax2.plot(dat.t, dat.w_dot, linewidth=0.1, color='blue', label='Power Output')
            ax1.set_xlabel("Timestep (hr)")
            ax1.set_ylabel("Distillate (kg/s)")
            ax2.set_ylabel("Power Output (MW)")
            ax1.set_xlim([2500, 2750])
