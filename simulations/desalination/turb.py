"This model generates data for different turbine sizes ranging from 100% base case to 200% base case."
...
W_dot_nom                                         = 494 
sens_turb                                         = [1.0*W_dot_nom, 1.1*W_dot_nom, 1.2*W_dot_nom, 1.3*W_dot_nom, 1.4*W_dot_nom, 1.5*W_dot_nom, 1.6*W_dot_nom, 1.7*W_dot_nom, 1.8*W_dot_nom, 1.9*W_dot_nom, 2.0*W_dot_nom]
...
for turb in sens_turb:
    ...
    def constr_cmass(model, t):
        return model.w_dot[t]                    == model.eta[t]*model.K_chpt * (model.alpha*model.m_dot_hpt[t] - model.beta*model.m_dot_e[t] - model.K_cint*m_dot_csteam)
    ...
    title_str = 'turb' + str(10*(9+counter_turb)) + '.csv'
