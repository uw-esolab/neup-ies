...
class MED:
    ...
    def __init__(self):
        ...
        for i in range(0, self.k):
            ...
            self.enth_feed =  SeaWater(T=self.feed_temp+273.15,S=self.feed_conc,P=self.pressure[0]).h
        ...
        for j in range(1,self.k):           #Creating the first row of the matrix
            A[0,j] = (-self.max_brine_conc/(self.max_brine_conc-self.brine_conc[j-1]))*\
        ...
        for q in range(1,self.k):           #Filling in the rest of the matrix
            ...
            A[q,q]= -(self.enth_vapor[q] - self.enth_brine[q]) + \
                (self.max_brine_conc/(self.max_brine_conc-self.brine_conc[q]))*\
                    (self.enth_brine[q-1] - self.enth_brine[q])
            #Below is on column to the left of the diagonal of the matrix
            A[q,q-1]= self.latentheat[q-1]+((self.max_brine_conc/(self.max_brine_conc-self.brine_conc[q]))-1)*\
                (self.enth_brine[q-1] - self.enth_brine[q])
            for j in range(0,q-1):
                #Below is everything to the left, under the q-1 diagonal. 
                A[q,j]= (self.max_brine_conc/(self.max_brine_conc-self.brine_conc[q])-1)*\
                        (self.enth_brine[q-1] - self.enth_brine[q])
            for j in range(q+1,self.k):
                #Below is everything to the right of the q diagonal
                A[q,j]= (self.max_brine_conc/(self.max_brine_conc-self.brine_conc[q]))*\
                        (self.enth_brine[q-1] - self.enth_brine[q])
