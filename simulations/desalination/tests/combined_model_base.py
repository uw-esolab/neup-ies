...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 21 09:23:03 2023

@author: elizabethkeith
"""

"For this test case, the price at which water is sold at is set to 0, as is the penalty associated \
    with ramping the NPP, results show that power generation stays at the nominal 494 MW except when pricing periods \
        are negative - fluctuations are due to changes in ambient efficiency"
...
cost_molten_salt_TES_kWh          = 29.8                                                                              # [$/kWh-th], from Gabriel's model
...
desal_price_per_kg                = 0.0021                                                                            # [$/kg]  


m_dot_salt                        = Q_dot_LFR*1e3 / (cp_salt * DeltaT_ctes)                                           #kg/s
m_dot_csteam                      = W_dot_gen/eta_cycle_ref*1e6 / (DeltaH_csteam*1e3)                                 #kg/s #nominal steam flow to hpt
...
model.C_desal_tot                  = pyomo.Var(domain=pyomo.NonNegativeReals)
#  ==================================================
#   Objective
def objective(model):
    ...
    return sum([model.P_elec[t]*model.w_dot[t]*model.Delta_t + model.P_dist[t]*model.v_dot[t]*3600*model.Delta_t for t in model.T]) \
        ...
        - (model.C_cs*model.M_cm_max)/30 - (model.C_ds*model.M_dm_max)/30 - model.C_desal_tot/30
...
def constr_C_desal_tot(model):
    return model.C_desal_tot       == model.C_desal * model.V_dot_max
model.constr_C_desal_tot           =  pyomo.Constraint(rule=constr_C_desal_tot)
...
def constr_vbounds_up(model, t):
    return model.v_dot[t]          <= model.V_dot_max*1000/(24*3600) 
...
outlabs = ['t', 'P_elec','eta','m_ch', 'm_dh', 'w_dot', 'v_dot', 'm_dot_cs','m_dot_hpt','m_dot_e','m_dot_ds', 'M_cm_max', 'M_dm_max', 'V_dot_max','C_desal_tot']
...
df_out.to_csv('results_base.csv', header=True)