...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 21 14:44:54 2022
...
csp_data_array   = Plots.q_rec_thermal_dict
nuc_data_array   = Plots.q_nuc_thermal_dict
...
gsQ1 = gridspec.GridSpecFromSubplotSpec(3, 2, subplot_spec=gsQ[0])
gsQ2 = gridspec.GridSpecFromSubplotSpec(3, 2, subplot_spec=gsQ[1])
...
for i, dayOfWeek in enumerate(["WeekDay", "WeekEnd"]):
    for j, season in enumerate(["Winter", "Summer"]):
        ...
        plotting_csp_array = csp_data_array[season][dayOfWeek].tolist()
        plotting_nuc_array = nuc_data_array[season][dayOfWeek].tolist()
        ...
        Plots.create_tariff_overlay( csp_ax__, is_winter, is_weekday, s_color=s_color)
        ...
        Plots.create_violin_plot(  tch_ax__, plotting_tch_array, tch_data_array, v_color=tch_color, hide_x=True )
        Plots.create_violin_plot(  csp_ax__, plotting_csp_array, csp_data_array, v_color=csp_color, hide_x=hide_x )
        Plots.create_violin_plot(  csp_ax__, plotting_nuc_array, nuc_data_array, v_color='darkblue', hide_x=hide_x )
        ...
        if j == 0:
            ...
            csp_ax__.set_ylabel("CSP \nPower \n(MWt)", fontsize=14, fontweight='bold')
