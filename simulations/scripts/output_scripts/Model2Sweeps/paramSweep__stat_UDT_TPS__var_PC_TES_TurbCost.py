...
for json in ['model2_CAISO_Hamilton_mod']: #[: #model2_Hamilton_560_tariffx1_mod'
    ...
    for root_case in ['sweep','large']: #['nuclear'] solar 'micro','small2','small1'
        ...
        if syn:
            ...
            if case == 'solar':
                ...
                if "CAISO" in json:
                    ...
                    p_cycle    = np.array([190,170,150,130,110])
                else:
                    tshours    = np.array([ 0.1,1,2])
                    p_cycle    = np.array([310,290,270,250,230])
                
            elif case=="small2":
                q_dot_nuclear_des=100
                if "CAISO" in json:
                    tshours    = np.array([9,10,11,12,13])
                    p_cycle    = np.array([270,250,230,210,190])
                else:
                    tshours    = np.array([ 0.1,1,2])
                    p_cycle    = np.array([350,330,310,290,270])
                
            elif case == "small1":
                q_dot_nuclear_des=250
                if "CAISO" in json:
                    tshours    = np.array([7,8,9,10,11])
                    p_cycle    = np.array([400,380,360,340,320])
                else:
                    tshours    = np.array([ 0.1,1,2])
                    p_cycle    = np.array([420,400,380,360,240])
                ...
                if "CAISO" in json:
                    ...
                    tshours    = np.array([ 0.1,1,2])
                    p_cycle    = np.array([810,760,710,660,610]) 
        ...
        filename = 'paramSweep_varTurbineCost_PySAM__{0}__2022_05__pyomo_{1:.0f}__horizon_{2:.0f}_{3:.0f}__TES_[{4},{5}]__PC_[{6},{7}]__{8}.nuctes'.format(
