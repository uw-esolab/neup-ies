...
for model1 in [True,False]:
    ...
    if model1:
        raise ValueError("Currently disabled as I am reverting to using model1: solar '1' isnt fully removed I think")
    
    for json in ['model2_CAISO_Hamilton_mod']: #'model2_Hamilton_560_tariffx1_mod' 
        
        for root_case in ['small2','sweep','small1','small2','micro','large']: #['zero'
            
            tmp_ppa=None
            
            if root_case == 'zero' and model1:
                continue
            ...
            if syn:
                raise ValueError("Not yet reconfigured")
            ...
            if model1:
                #sweep is not necessary. Note: should be no change vs actual Model1 as 0,0 is optimal
                if case=='sweep':
                    q_dot_nuclear_des=950
                elif case=='large':
                    q_dot_nuclear_des=1900
                ...
                if json == "model2_Hamilton_560_tariffx1_mod":
                    p_cycle    = np.array([ int(450.0/950*q_dot_nuclear_des)]) 
                    tshours    = np.array([ 0 ])
                else:
                    p_cycle = q_dot_nuclear_des/950*np.array([900,850,800,750,700,650,600,500,550,450])
            ...
            filename = 'paramSweep_varTurbineCost_PySAM__{0}__2022_05__pyomo_{1:.0f}__horizon_{2:.0f}_{3:.0f}__TES_[{4},{5}]__PC_[{6},{7}]__{8}__{9}.nuctes'.format(
